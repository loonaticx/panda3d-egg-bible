...
target. See the `Morphs`_ section below.
...
calculated for `CollisionPolygons <https://docs.panda3d.org/1.10/python/reference/panda3d.core.CollisionPolygon>`_.
...
`GeomPatches <https://docs.panda3d.org/1.10/python/reference/panda3d.core.GeomPatches>`_ object to be delivered to the shader in a single batch.
...
Triangle strips are rarely encountered in an egg file; it is
...
`ModelNode <https://docs.panda3d.org/1.10/python/reference/panda3d.core.ModelNode>`_.

- If the string is ``local``, it indicates that the local
  ...
  the entire model loading process.

- If the string is ``net``, then
...
particular group.  This creates a `ModelNode <https://docs.panda3d.org/1.10/python/reference/panda3d.core.ModelNode>`_ at the corresponding
...
the high-level `Actor <https://docs.panda3d.org/1.10/python/reference/direct.actor.Actor>`_ class, will be created for this group.
...
In the absence of an ``fps`` scalar for the group (see below), the egg loader creates a `SwitchNode <https://docs.panda3d.org/1.10/python/reference/panda3d.core.SwitchNode>`_, and it
...
`SequenceNode <https://docs.panda3d.org/1.10/python/reference/panda3d.core.SequenceNode>`_ instead, which automatically cycles through its
children.
...
``<S$Anim>`` entries, one for each named morph.  Each table
...
retrievable via `NodePath::get_tag() <https://docs.panda3d.org/1.10/python/reference/panda3d.core.NodePath#panda3d.core.NodePath.getTag>`_ and related interfaces, on
this node.